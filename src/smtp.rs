...
lazy_static! {
  ...
  static ref CMD_REGEX: Regex = {
    ...
    let cmds: &[&str] = &[&helo_cmd, &ehlo_cmd, &mail_cmd, &rcpt_cmd, data_cmd, rset_cmd, vrfy_cmd];
...
impl Session {
  pub fn new(fd: RawFd, tmpdir: Arc<TempDir>, logger: Arc<Logger>, origin: Arc<String>, timeout: Arc<ITimerSpec>) -> Result<Session> {
  ...
  pub fn ready(&mut self) -> Result<()> {
    ...
    try!(self.send_reply(ReplyCode::ServiceReady, "Service ready"));
  ...
  pub fn not_ready(mut self) {
    ...
    // This is not fully RFC-compliant -- in fact, there is no defined way
      ...
      _ => {
        ...
        match str::from_utf8(buf) {
          Ok(sbuf) => {
            ...
            // XXX: this should be optimal (String::new just clears pointers), but better
            // check later.
  ...
  fn log(&self, level: char, s: &str) {
    // We better crash loudly if there's something wrong with the logging
    self.logger.log(level, s).unwrap()
  }

  fn send_reply(&mut self, code: ReplyCode, msg: &str) -> Result<()> {
    let mut mmsg = msg;
    while let Some(pos) = mmsg.find('\n') {
      let (curr, next) = mmsg.split_at(pos);
      try!(unistd::write(self.fd, format!("{}-{}\r\n", code as u16, curr).as_bytes()));
      mmsg = next;
    }
    try!(unistd::write(self.fd, format!("{} {}\r\n", code as u16, mmsg).as_bytes()));
    self.log('I', &format!("sent reply: {} {}", code as u16, msg));
    Ok(())
  }
  ...
  fn command_step<'tt>(&mut self, cmd: &SMTPCommand<'tt>) -> Result<(Reply, CommandResult)> {
    ...
    match *cmd {
      ...
      SMTPCommand::Data if self.status == Status::InSession && self.tos.len() > 0 => {
        self.tmpfile = Some(try!(NamedTempFile::new_in(self.tmpdir.path())));
  ...
  fn chunk_step(&mut self, chunk: &str) -> CommandResult {
    ...
    self.tmpfile = None;
      ...
      _ => {
        ...
        match self.send_reply(code, &*reply) {
          Ok(()) => (connected, msg),
          Err(err) => {
